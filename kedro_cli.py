...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/ibm_watson_recommendation_engine")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=ibm_watson_recommendation_engine"]
    )
    ...
    call(
        [
            ...
            "-o",
            "docs/source",
            "src/ibm_watson_recommendation_engine",
        ]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "ibm-watson-recommendation-engine"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "ibm_watson_recommendation_engine"
